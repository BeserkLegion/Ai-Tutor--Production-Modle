...
# JSON template appended to every custom prompt so the model always
# returns the four keys the result screen depends on.
CUSTOM_PROMPT_JSON_INSTRUCTION = (
    "\n\nIMPORTANT: Return ONLY valid JSON with no extra text, no markdown, "
    "and no code fences. The JSON must contain exactly these four keys:\n"
    "  \"score\"     — the total mark converted to a percentage (0–100 integer). "
    "If the rubric is out of 12, divide by 12 and multiply by 100, rounded to the nearest integer.\n"
    "  \"strengths\" — a list of strings describing what the student did well.\n"
    "  \"weaknesses\"— a list of strings describing what the student needs to improve.\n"
    "  \"feedback\"  — a single string with targeted, criterion-by-criterion feedback.\n\n"
    "Example (for a rubric out of 12 where the student scored 9):\n"
    '{"score": 75, "strengths": ["Good use of ethical framework", "Identified key stakeholders"], '
    '"weaknesses": ["Missing King V discussion"], "feedback": "Your ethical framework analysis was strong..."}'
)


def _safe_parse_result(raw_text: str, custom_prompt: bool = False) -> dict:
    """
    Parse the model's raw text into the expected result dict.
    Handles:
      - JSON wrapped in markdown code fences
      - Missing keys (score, strengths, weaknesses, feedback)
      - Non-standard score keys (total, total_out_of_12, raw_score, mark)
      - Numeric score outside 0-100 (assumes it's out of the rubric max and scales it)
    """
    ...
    try:
        ...
        # Last-resort: return a safe default so the app never crashes
        return {
            "score": 0,
            "strengths": [],
            "weaknesses": [],
            "feedback": (
                ...
                "Please try submitting again. If the problem persists, contact your lecturer."
            ),
        }

    # ── Normalise score ────────────────────────────────────────────────────────
    if "score" not in data:
        # Look for alternative keys the model may have used
        for alt in ("total", "total_out_of_12", "total_mark", "raw_score",
                    "mark", "total_score", "percentage"):
            if alt in data:
                data["score"] = data[alt]
                break
        else:
    ...
    # If the score looks like it's on a rubric scale rather than 0-100,
    # scale it up. We treat any value <= 20 coming from a custom prompt
    # as likely being out of a small rubric (e.g. out of 12).
    try:
        raw_score = float(data["score"])
    except (TypeError, ValueError):
        raw_score = 0.0

    if custom_prompt and raw_score <= 20:
        # Assume out of 12 (the most common rubric in this app); scale to 100
        data["score"] = round((raw_score / 12) * 100)
    else:
        data["score"] = max(0, min(100, round(raw_score)))
...
def grade_answer(scenario_text, question_text, student_answer, custom_prompt=""):
    ...
    if is_custom:
        ...
        prompt += CUSTOM_PROMPT_JSON_INSTRUCTION
    else:
    ...
    return _safe_parse_result(response.output_text, custom_prompt=is_custom)
        ...
        if not student_id.strip():
            ...
            with st.spinner("Analysing your answer..."):
                try:
                    ...
                    st.error(
                        "⚠️ Grading failed due to an unexpected error. "
                        "Please try again or contact your lecturer if the problem persists."
                    )
                    st.stop()
            ...
            st.session_state.result = {
                "score":      result["score"],
                "strengths":  ", ".join(result["strengths"]) if result["strengths"] else "See feedback below.",
                "weaknesses": ", ".join(result["weaknesses"]) if result["weaknesses"] else "See feedback below.",
            ...
            try:
                log_submission(
                    ...
                    strengths     = st.session_state.result["strengths"],
                    weaknesses    = st.session_state.result["weaknesses"],
                    feedback      = result["feedback"],
                    email         = logged_email,
                )
            except Exception:
                pass  # Logging failure should never block the student seeing their result
